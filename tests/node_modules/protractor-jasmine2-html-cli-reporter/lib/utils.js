...
module.exports = (() => {
    ...
    utils.getCliReportOptions = options => {
        let opts = options || {};
    ...
    utils.getOption = options => {
        let UNDEFINED;
        // sanitize arguments
        let opt = options || {};
        opt.takeScreenshots = options.takeScreenshots === UNDEFINED ? true : options.takeScreenshots;
        opt.savePath = options.savePath || '';
        opt.takeScreenshotsOnlyOnFailures = options.takeScreenshotsOnlyOnFailures === UNDEFINED ? false : options.takeScreenshotsOnlyOnFailures;
        opt.screenshotsFolder = 'assets/' + ((options.screenshotsFolder || 'screenshots').replace(/^\//, '') + '/');
        opt.useDotNotation = options.useDotNotation === UNDEFINED ? true : options.useDotNotation;
        opt.fixedScreenshotName = options.fixedScreenshotName === UNDEFINED ? false : options.fixedScreenshotName;
        opt.consolidate = options.consolidate === UNDEFINED ? true : options.consolidate;
        opt.consolidateAll = options.consolidate !== false && (options.consolidateAll === UNDEFINED ? true : options.consolidateAll);
        opt.fileNameSeparator = options.fileNameSeparator === UNDEFINED ? '-' : options.fileNameSeparator;
        opt.fileNamePrefix = options.fileNamePrefix === UNDEFINED ? '' : options.fileNamePrefix;
        opt.fileNameSuffix = options.fileNameSuffix === UNDEFINED ? '' : options.fileNameSuffix;
        opt.fileNameDateSuffix = options.fileNameDateSuffix === UNDEFINED ? false : options.fileNameDateSuffix;
        opt.fileName = options.fileName === UNDEFINED ? 'htmlReport' : options.fileName;
        opt.cleanDestination = options.cleanDestination === UNDEFINED ? true : options.cleanDestination;
        opt.showPassed = options.showPassed === UNDEFINED ? true : options.showPassed;
        return opt;
    }
